...
def compute_tier(event: dict, size: int, tier: dict, pkg: dict, skus: dict):
    ...
    if "cocktails-with-spandex" in tier.get("extras", []):
        ...
        rows.append({
            "key": "cocktail-table",
            "qty": n_cocktail,
            "unit_price": skus["cocktail-table"]["startingPriceCAD"],
            "label": "Cocktail Highboy Tables",
        })
        rows.append({
            "key": "cocktail-spandex-cover",
            "qty": n_cocktail,
            "unit_price": pkg["spandexPackagePrice"],
            "label": f"Spandex Cocktail Covers (${pkg['spandexRetailPrice']}/table value)",
        })
...
def build_context(event_slug: str, event: dict, size: int, pkg: dict, skus: dict) -> dict:
    tier_cards = [compute_tier(event, size, t, pkg, skus) for t in pkg["tiers"]]
...
def main():
    ...
    for event_slug in events:
        ...
        for size in sizes:
            ctx = build_context(event_slug, event, size, pkg, skus)
