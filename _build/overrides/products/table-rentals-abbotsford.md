---
slug: table-rentals-abbotsford
title: "Table Rentals in Abbotsford — Round, Banquet & Cocktail"
meta_description: "Table rentals in Abbotsford — 5ft round, 6ft and 8ft banquet, cocktail highboys delivered and positioned to your floor plan by the local crew. 190+ reviews."
hero_subtitle: "Table rentals in Abbotsford — 5ft round tables for receptions, 6ft and 8ft banquet tables for head, gift, and buffet, cocktail highboys for cocktail hour. Positioned to your floor plan, no setup surcharge."

intro_paragraphs:
  - "Abbotsford table rentals serve the widest table-volume range of any city we deliver to — 80-guest Clayburn Village heritage receptions, 150-guest vineyard barn dinners on Mount Lehman, 500-guest Tradex Exhibition Centre galas, Mill Lake Park civic events, and acreage farm receptions across Matsqui and Sumas Mountain. The table count and mix changes between them. Tradex galas need 50+ banquet registration tables and high cocktail-highboy counts; vineyard weddings run the standard rounds-and-banquets mix with barn-style head tables; heritage Clayburn weddings stay small."
  - "Round, banquet, cocktail highboy — quoted up front, delivered on a one-hour window, positioned to your floor plan by the same crew that loaded it. We don't sub-contract the install."

faqs:
  - q: "What's included on an Abbotsford table rental quote?"
    a: "Every Abbotsford table quote we send is itemised: tables by line, delivery, crew positioning to your floor plan, optional add-ons, and applicable taxes. No setup surcharge — every table is unfolded and positioned during the delivery window."
  - q: "How many tables for a 100-guest Abbotsford wedding?"
    a: "For seated dinner at 5ft rounds, plan ~12 rounds (8 guests per round = 96; round up for kids and split parties). Add 5–8 banquet tables for head, gift, cake, DJ, registration, and buffet. Add 2–3 cocktail highboys for the pre-dinner cocktail hour. Vineyard barn weddings often link 2–3 banquets into a long farm-table head — that adjusts the count slightly."
  - q: "Can you deliver tables to a vineyard or to Tradex?"
    a: "Yes — both are routine Abbotsford bookings. Vineyard deliveries use rural-road truck access with on-site staging. Tradex Exhibition Centre's loading dock is ample and we coordinate the delivery window during quoting; gala-scale orders (50+ banquets, 30+ highboys) typically ship on a dedicated truck."
  - q: "Do you supply linens, runners, or chargers?"
    a: "We don't stock linens directly, but we coordinate through our preferred Lower Mainland linen partners and bundle them onto the same delivery window if you want. Chargers, runners, and place settings sit with the catering provider."
  - q: "Can you re-stage tables mid-event for a vineyard ceremony-to-reception flip?"
    a: "Yes — for any single-tent vineyard or farm ceremony-to-reception flip, our crew handles the table re-stage during the cocktail hour. The 6ft banquet tables fold flat and re-position quickly."

related_blog_posts:
  - slug: party-rental-checklist-50-100-150-200-guests
    title: "Rental Checklist for 50, 100, 150 & 200 Guests"
  - slug: how-much-space-per-guest-wedding-tent
    title: "How Much Square Footage Per Guest?"
  - slug: tent-size-guide-lower-mainland-wedding
    title: "Tent Size Guide for Lower Mainland Weddings"

nearby_cities:
  - slug: chilliwack
    name: Chilliwack
  - slug: mission
    name: Mission
  - slug: aldergrove
    name: Aldergrove
---

## What our Abbotsford table lineup covers

![A formal round dinner table with full place settings, candles, and a floral centerpiece](/images/lifestyle/round-table-dinner-1.jpg)
*A 5ft round set for 8 — the Abbotsford wedding-reception standard.*

Three tables, three jobs. Abbotsford event floor plans usually need all three:

- **[5ft round tables](/product-round-table-5ft) ($13.50/day, seats 8)** — the classic wedding-reception configuration. Eight guests, room for centerpieces, fits two-deep into most marquee footprints (a 20×40 holds 6 rounds; a 20×60 holds 12). Standard for vineyard and farm marquee receptions.
- **[6ft rectangular banquet tables](/product-banquet-table-6ft) ($10.95/day, seats 6–8)** — the single most versatile SKU we ship. Family-style dining rows in vineyard barns, buffet stations, gift tables, DJ booths, registration desks at Tradex galas. Under 75 lb so they re-stage quickly mid-event. Linked banquets are a vineyard-barn favourite for farm-table head configurations.
- **[Cocktail highboy tables](/product-cocktail-table) ($15.50/day, standing)** — 42-inch standing-height rounds for cocktail hour, networking events, and pre-dinner mingling. Plan ~1 highboy per 10–12 standing guests. Tradex corporate events often run 30+ highboys.

> Tables care most about access and floor plan — we don't add a per-table setup charge when delivery is on the quote.

## Abbotsford venue access and floor-plan staging

![Banquet tables with white linens and fanback folding chairs set for a reception inside a frame tent](/images/lifestyle/tent-reception-tables-fanback-chairs.jpg){.float-right}
*Banquet rows and fanback chairs dressed under a frame tent.*

### Vineyards and farms — Mount Lehman, Matsqui

Standard rounds-and-banquets mix, often with linked banquets for farm-table head configurations. Truck access is ample on rural roads; we stage tables in the barn or under the marquee during the delivery window.

### Tradex Exhibition Centre — gala-scale corporate

Tradex galas run the largest single-event table counts on our calendar — 50+ banquet registration tables and 30+ cocktail highboys is routine. Loading dock access is ample.

### Mill Lake Park and civic

Mill Lake Park requires city event-permit coordination. Standard table mix; smaller civic gatherings usually run banquet-and-cocktail-highboy configurations rather than rounds.

### Clayburn Village heritage

Smaller, more intimate weddings (60–100 guests). Tight heritage streets constrain delivery; we use shorter delivery windows and pre-stage closer to the property line.

### Sumas Mountain and McKee acreage

Standard residential backyard patterns with longer driveways. Most acreage weddings take the full table mix at 100–200 guests.

### Per-item pricing

The same on a vineyard as at Tradex. Our 5ft round table is one rate, full stop. The line that changes between bookings is delivery — never the per-table rate.

## Mid-event flips and ceremony-to-reception staging

Many Abbotsford vineyard and farm weddings run a single-tent ceremony-to-reception flip — same chairs, same marquee, with our crew rearranging the table layout in 30–45 minutes during the cocktail-hour window. Banquet tables fold flat and re-position quickly; rounds re-stack to the truck while the dinner configuration goes up. This is included on single-tent bookings, not an add-on line.

## A 100-guest Abbotsford table rental order

For a 100-guest Abbotsford wedding with a vineyard barn reception and a separate cocktail-hour space, a typical table-side order looks like:

- 12 × **5ft round tables** (8 guests per round = 96, with 2 extras for kids and overflow)
- 6 × **6ft banquet tables** (head linking 2 in farm-table configuration, gift, cake, DJ, registration)
- 2 × **6ft banquet tables** for the buffet line
- 3 × **cocktail highboys** for the pre-dinner cocktail hour

Total: 23 tables, all positioned to your floor plan during a single delivery window. For Tradex galas (250+ guests), the table count and mix shift toward banquet-and-highboy configurations rather than rounds. The [50/100/150/200 rental checklist](/blog/party-rental-checklist-50-100-150-200-guests) maps the table mix to your headcount.

## Why couples book us a second time

We're a small enough team that the person who quotes your event is on the truck the morning of. We're large enough that we own every line item — tent, chair, table, dance floor, lighting — and don't sub-rent.

190+ five-star Google reviews. One number to call from quote to teardown. A quote you can read top-to-bottom in under a minute. That's the service.
