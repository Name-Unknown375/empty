...
INFO_PAGES = {
    "service-areas.html", "faq.html", "testimonials.html",
    "contact.html", "corporate.html",
}


def priority_and_freq(filename: str, tier_map: dict[str, int]) -> tuple[float, str]:
    """Return (priority, changefreq) for a given generated filename."""
    ...
    # (Anything else generated, treat as product-city.)
    return 0.8, "weekly"


def collect_files() -> list[Path]:
    """Return sorted list of *.html files in SITE_DIR, excluding EXCLUDED."""
    files = [p for p in SITE_DIR.glob("*.html") if p.name not in EXCLUDED]
...
def build_sitemap(tier_map: dict[str, int]) -> str:
    ...
    for path in files:
        ...
        priority, changefreq = priority_and_freq(path.name, tier_map)
        lines.append("  <url>")
