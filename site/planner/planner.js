...
(function () {
  ...
  // ── Constants ──────────────────────────────────────────────────────────
  // SAVE_VERSION 2 keeps the eventDays field in the schema for forward
  // compatibility, but FPR's pricing model treats a 1–3 day weekend rental
  // as a single rental at the per-day rate. The duration toggle was
  // removed; eventDays is pinned to 1 and the per-day rate IS the rental
  // rate. If multi-week pricing ever lands, restore the toggle.
  ...
  function renderPalette() {
    const groupsHTML = catalog.groups.map(g => `
      <div class="pl-group">
        ...
        <div class="pl-tiles">
          ${g.items.map(it => `
            <div class="pl-tile" draggable="true" data-key="${it.key}" title="${it.label}">
  ...
  // TENT — dashed-stroke rectangle (legacy look kept; tent in plan view
  // is just an outline since the sides aren't structural).
  function drawTentSprite(g, sz) {
    svg('rect', {
  ...
  function buildLineItems() {
    ...
    // Sort: tents first (largest visual + highest price), then tables, chairs, floors
    const order = { tent: 0, danceFloor: 1, circle: 2, rect: 3 };
    lines.sort((a, b) => {
  ...
  function renderQuote() {
    ...
    const rows = [];
    if (perDay > 0)   rows.push(`<div class="pl-quote-totals-row"><span>Tables, chairs &amp; pickup items</span><span>${fmtMoney(perDay)}</span></div>`);
  ...
  function setupCanvasInteractions() {
    ...
    dom.svg.addEventListener('pointermove', e => {
      ...
      if (resizing) {
        ...
        // so relative spacing within the group is preserved.
        if (e.shiftKey) {
  ...
  //     with trailing default fields stripped.
  //
  ...
  function encodeStateForUrl() {
    ...
    const itemEncs = s.items.map(it => {
      ...
      // text-label extras (text, widthFt, depthFt, fontSize). Empty slots
    ...
    try {
      ...
      if (!isExternalEmbed) {
        ...
        if (lines.length) {
          ...
          for (const l of lines) {
            grand += l.subtotal;
            const unitLabel = l.unit === 'day' ? `${fmtMoney(l.unitPrice)} per rental` : `${fmtMoney(l.unitPrice)}/event`;
            writeLine(`${l.qty} ×  ${l.label}   —   ${unitLabel}   =   ${fmtMoney(l.subtotal)}`);
          }
  ...
  function buildPrintPage() {
    ...
    const { svg: layoutSvg } = buildExportSVG({ includeFooter: false });
    ...
    // matches the live cost panel exactly.
  ...
  function setupQuoteForm() {
    ...
    f.addEventListener('submit', async e => {
      ...
      const subtotalLines = [];
      if (perDay > 0)   subtotalLines.push(`  Tables, chairs & pickup items   ${fmtMoney(perDay)}`);
